<template>
    <div v-if="loaded" class="flex flex-col justify-between">
        <header class="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm flex justify-between">
            ...
            <IQSToolsRead v-if="winFormat.Header.type != 5 && props.modal == false" :disable-first="disableFirst" :disable-prev="disablePrev"
...
async function loadData() {
    ...
    if (id === "new") {
        ...
        if (winFormat.value.Details.length > 0) {
            for (var detail of winFormat.value.Details) {
                const detailData = await getInfoDetails(windowName, Number(detail.value), headerData[0][winFormat.value.Header.primaryKey], winFormat.value);
                const detailModel = {
