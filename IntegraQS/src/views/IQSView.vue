...
async function initializeWindow(): Promise<void> {
  ...
  instanceId.value = windowStore.registerWindow({
    ...
    initialState: currentId.value === "new" ? "creation" : "read",
  });
...
async function loadData() {
  ...
  if (id === "new") {
    ...
    windowStore.setWindowState(instanceId.value ?? "", "creation");
  } else {
    // 2. Datos de cabecera
...
async function onSave() {
  ...
  if (currentWindow.value == "creation") {
    const windowName = currentWindow.value;
...
function onChange(newModel: DynamicModel) {
  if (currentWindow.value !== "modify" && currentWindow.value !== "creation") {
